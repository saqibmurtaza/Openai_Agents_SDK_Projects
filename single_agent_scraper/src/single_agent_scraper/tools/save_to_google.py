...
# import logging
# import traceback
# from gspread.exceptions import WorksheetNotFound, NoValidUrlKeyFound # Import NoValidUrlKeyFound
# from gspread.utils import extract_id_from_url # Import the extraction function

# # Assuming 'client' is already initialized and authenticated
# # client = gspread.service_account(filename='path/to/your/credentials.json')

# # @function_tool
# def save_to_sheet(client: gspread.Client, sheet_url: str, products: list, tab_name: str = "Products") -> str:
#     """
#     Saves product data to a Google Sheet. Extracts sheet ID from the URL.
#     Args:
#         sheet_url: The URL of the Google Sheet
#         products: List of product dictionaries to save
#         tab_name: Name of the worksheet tab (default: Products)
#     Returns:
...
#         spreadsheet = client.open_by_key(sheet_id) # Use open_by_key with the extracted ID
...
#         # You might want to adjust rows and cols based on your expected data size
#         worksheet = spreadsheet.add_worksheet(title=tab_name, rows="100", cols="20")

#         # Convert data to DataFrame and update sheet
#         df = pd.DataFrame(products)
#         # Ensure column order if needed, otherwise it uses dictionary key order
#         # worksheet.update([df.columns.values.tolist()] + df.values.tolist())
#         # To update with headers and data:
#         worksheet.update([df.columns.values.tolist()] + df.values.tolist())


#         return f"Successfully saved {len(products)} products to '{tab_name}' worksheet"

#     except NoValidUrlKeyFound: # Catch the specific exception for invalid URLs
#         error_msg = f"Failed to save to Google Sheet: The provided URL does not contain a valid Google Sheet ID. Please check the URL."
